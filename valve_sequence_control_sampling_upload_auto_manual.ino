...
#define OPEN LOW  //0 V
// valve state
enum valvestate {IDLE =0,DRAIN_BEFORE,CLEAN_BEFORE,FILL,SAMPLE,DRAIN_AFTER,CLEAN_AFTER};
static valvestate valst; //valve state
    ...
    while (Serial.available() > 0) {
      ...
      switch(inChar){
        ...
        // else if(inChar == 's'){ // manual sampling by user command
          state = 's';
           valst = DRAIN_BEFORE;
          Serial.print(F("valvestate auto: "));
        ...
        // else if(inChar == 'm'){ // manual check for each state
          ...
          Serial.print(F("valvestate manual: "));
          Serial.println(valst);

          valve_sequence(0, valst);
          break;
        // }
...
// void valve_sequence(int valvestate){// sampling

  Serial.print("valst before:");
  Serial.print(valst);


  Serial.print(" valst increase after:");
  Serial.println(valst);
